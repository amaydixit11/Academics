...
## ♟️ 2. NIM GAME (MINIMAX & ALPHA-BETA PRUNING)
...
## ♛ 3. N-QUEENS PROBLEM (LOCAL SEARCH)
...
def solve_nqueens(initial_board=None, max_steps=500):
    ...
    while True:
        ...
        while steps < max_steps:
            if conflicts == 0:
                print(f"✓ Solution found in {steps} steps, {attempts} restarts")
                return board
...
## 🌡️ 4. SIMULATED ANNEALING (8-QUEENS)
...
- Change puzzle size (3x3 → 4x4)
- Change game rules (nim → custom)
- Change initial state

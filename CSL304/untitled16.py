...
# A* with trivial admissible heuristic (h=0) -> reduces to Dijkstra (we still implement general A*)
def astar(start, target, caps):
    start = round_state(start)
    target = round_state(target)
    # Heuristic: trivial 0 (admissible). We return path if found.
    def heuristic(s):
        # minimal number of moves required if each move can at best fix one reservoir
        # (sum of absolute differences divided by the max transfer possible)
        diff = sum(abs(si - ti) for si, ti in zip(s, target))
        max_cap = max(caps)
        max_transfer = max_cap * 0.8      # 80% of some reservoir can be moved at most
        return math.ceil(diff / max_transfer)


    open_heap = []
    gscore = {start: 0}
    parents = {start: (None, None)}  # state -> (parent_state, action)
